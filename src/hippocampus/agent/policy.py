...
# 只有这两个通道的分数是"内容相关性"度量，能作为"有依据"的凭据；
# 图（离散分+会话实体兜底）与事件线索（规则命中给固定分）都不算——见 has_evidence 的说明。
_EVIDENCE_CHANNELS = frozenset({"semantic", "bm25"})
...
class RulePolicy:
    ...
    def has_evidence(self, memory_lines: list[str], scored: list[tuple[float, str]] | None) -> bool:
        ...
        两条都要，缺一不可：
        - **分数闸**：只认**衡量内容相关性**的两个通道——语义（semantic）与关键词（bm25）。
          另两个通道的分数不是相关性度量，不能单独授权作答：
          · 图通道是离散值（0.5/0.3/0.15）＋会被"会话实体兜底"带上本会话的实体 →
            只表示"跟某个实体有关"；
          · 事件线索通道更极端：它是**规则命中**（线索词/会话/实体/时间），命中就给固定高分，
            问一句无关的话也可能捞到本会话的经历 → 曾导致"该拒答却作答"的间歇失败
            （测试里出现过：问"帮我写一首关于秋天的诗"被判成有依据）。
          两个通道捞出来的东西**可以注入**（那是"跨会话接着做"的能力），但不构成依据。
        - **重合闸**：还必须与问题有实义关键词重合（排除指代词与功能字），
          避免"捞到了但没回答问题"。
        """
        if not memory_lines or not scored:
            return False
        best = max((s for s, ch in scored if ch in _EVIDENCE_CHANNELS), default=0.0)
        if best < self.floor():
            return False
