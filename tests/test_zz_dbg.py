...
**终态注记（九轮 W14）**：八轮曾请示"是否更名／并入 `test_n2x` 正式序列"，栗子未答复 →
按"无答复只加注记、不动文件"处理：**文件名与内容一律保持原样**。理由＝这条用例是
「挂起 → 裁决 → 注入集合更新」整条链的唯一回归，改名会打断 `git log`/CI 历史里的可追溯性，
而收益只是命名整齐。**翻案触发条件**：下次有人**实质修改**本文件内容时，随那次改动一并
改名为 `test_n47_r*_pending_promote_inject.py` 并在此登记（同一次改动里做，不单独开一次纯改名提交）。
"""
