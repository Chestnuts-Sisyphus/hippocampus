...
> **终态口径（九轮 W14）**：本项目的**安全 seal 正本＝本节**（`pip-audit` 联网 OSV 复扫＋使用面
> 可达性判定＋凭据扫描＋CI 四条），**每次发版随 `scripts/` 的闸一起重跑一遍**，不沿用旧结论。
> Mimosa 是另一个工具（ZCode 的 PreToolUse 钩子）的产物，**不在本仓交付链里**，因此不再挂
> "等它可用再复扫"的悬空待字——那等于把一个外部工具的可用性当本项目的前置条件。
> 翻案条件（写明以便未来判定）：若 Mimosa 成为可独立调用的分发工具，则把它**叠加为第二 seal**
> （两路公告库交叉核对），本节仍作第一 seal 保留。此前所有"Mimosa 未刷新"的记录保留为历史。
