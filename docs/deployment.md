...
2. **鉴权分两档（八轮 V8 收紧后）**：
   ...
   · `GET /health` **绑环回时免鉴权**（自 0.1.0 的探针位，实测不带令牌回 200，本机拨测方便）；
     **一旦绑到非环回（`--allow-remote`）就同样要令牌，缺令牌 401**——回体里的计数／索引／锁是实质信息面，
     挂到局域网裸奔不可接受。两态由 `tests/test_n35_r8_health_auth.py` 钉住（接线＋行为各一遍）。
