...
| 基准 | 题量 | 证据进上下文 | 答在文内 | 上下文 tokens | 延迟 p50 |
|---|---|---|---|---|---|
| LongMemEval-oracle（抽 200） | 200 | **98.0%** | 40.0% | 864 | 78.1 ms |
| LoCoMo-10（全量） | 1986 | **36.7%** | 14.7% | 1611 | 75.5 ms |

消融（LoCoMo 全量）：注入条数 8→20 几乎不动（36.7%→37.2%）；只进经历层掉到 27.7%
