...
| ~~模型口固化阶段遇"上游回体不是合法 JSON"→ 裸 500~~（九轮 W6 真机冒烟逮到；**十轮 X1/X3 已修**，GG1 于 2026-09-22 与 HEAD 对齐） | **已修（十轮 X1/X3，commit `87447ea`）——原"本轮不修"作废**：模型口 `/v1/chat/completions` 与管理口 `/run` 现**共用同一个 502 出口** `proxy/app.py _consolidate_502`（`app.py:171-196`，调用点 `:300-301`／`:364-365`／`:428-429`），回体带 `stage="consolidate"`、`type="consolidation_failed"` 与**已完成注入段**（`run_id`／`scope`／`injected`／`dropped`／`upstream_reply`），管理口另带 `form_hint` 可分辨形态；同轮补 X3 请求体上限闸（超 `max_body_bytes` → 413 带 `max_body_bytes`）。原链路（上游回纯文本 → `extract._extract_with_llm` 抛 `ValueError` → ASGI 500）已不复现。**未发版**：`v0.5.0` tag 是 `87447ea` 的祖先，本修复随下一个 tag 出去（原计划"与 W1/W3 一起进一次 minor 发版"仍待执行，代码层已落地） | 判据已成文且被测试钉住：`tests/test_n47_r10_proxy_hardening.py:34-59`（`test_model_port_consolidate_returns_502_with_injected`／`test_management_port_same_shape` 两形态同错同码同形状）；X3 的 413 在 `:61-87`。真机复现随 W6 入 CI：`.github/workflows/ci.yml:125` 跑 `scripts/live_proxy_smoke.py`（桩上游回体换纯文本即红）。口径见 `docs/deployment.md` §二·五第 3 条 |
