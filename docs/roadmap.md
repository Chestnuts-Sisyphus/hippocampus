...
| `GET /health` 免鉴权（管理口探针位）是否收紧 | **⏸ 待拍板（09-19 真机冒烟登记）**：`/run`／`/trace` 已强制实例令牌（缺则 401，实测）；但 `/health` **自 0.1.0 起就是免鉴权探针位**（不带令牌回 200，内容为端口／档位／计数／索引健康）。默认只绑环回，暴露面有限 | 要收紧只需一行鉴权复用＋两处测试调整；**收紧会动到既有测试与外部探活用法，故按"最小改动＋不擅自扩面"未改**。口径已按两档如实写进 `docs/deployment.md` §二·五 |
...
| ~~公开基准是"检索口径"不是官方分~~ | **已实现（版本号未升，随下一个 tag 一起发）**：`bench --model-arm` 显式开关接官方判分臂（prompt 与判分算法钉官方仓库 revision，见 `docs/benchmark.md` §二·三）；实测：LME-oracle 200 题官方准确率 **70.5%**（CI 63.8–76.4%）、LoCoMo-10 全量官方 F1 **32.55%**（CI 30.8–34.4%，deepseek-chat、temp 0、**输入＝记忆层注入 top-8**） | **输入口径＝8 条注入，不是全文** → 与"把整段对话喂给模型"的报分同表比须带脚注（`docs/benchmark.md` §二·三）；官方分要花钱跑（预算硬停默认 ¥30，实际按余额差如实报） |
