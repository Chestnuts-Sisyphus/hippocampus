...
## ② 出站 URL 默认只允许 https，且拒绝环回／私有／保留地址

**两条通道，规则不同**（这条区分很重要，写错了要么拦不住要么拦太狠）：

| 通道 | 入口 | 来源 | 规则 |
|---|---|---|---|
| **数据/模型提供的 URL** | `validate_outbound_url` | 工具参数、记忆内容里的链接、抓取目标 | **仅 https**（九轮 W3 收紧）；**拒绝** localhost／环回／私有／保留／链路本地／云元数据地址（含 IPv6）。明文 `http://` 公网出口需显式设 `HIPPOCAMPUS_ALLOW_PLAINTEXT_OUTBOUND=1`，**默认关** |
| **操作员配置的端点** | `validate_endpoint_url` | 模型 `base_url`、本代理监听地址 | 仅 http/https；**允许环回**（本地模型端点走 `http://127.0.0.1:11434` 合法） |
...
| 使用边界 | 这是**单机自用**的入口门禁，不是多用户鉴权：**令牌永不过期、没有吊销列表——这是设计边界，不是待办**。唯一的失效手段是"换令牌＝删 `<数据根>/instance_token` 后重启"（旧令牌当场失效，本机所有客户端需重读）。对外统一措辞：*"实例令牌是本机实例的静态门禁，用于挡住同机其他进程与误暴露到非环回的匿名访问；不承担会话生命周期管理，需要撤销即换令牌"*。**终态（九轮 W14）**：不做到期与吊销——多用户 token 生命周期属另一产品形态，与"单机自用＋环回默认"的定位冲突，做了反而诱导把它当服务用 |
| 免鉴权探针口（**仅环回**） | `/health`、`/v1/models`：只回本机元信息与配置的模型名，不含任何记忆内容。**绑非环回时这两个口也要令牌**（八轮 V8 管 `/health`、九轮 W1 补齐 `/v1/models` 与模型口），缺则 401；两形态的 `--host` 非环回都必须显式 `--allow-remote` 且有令牌，否则拒起 |
...
- **危险动作需确认**：写文件／出站抓取属危险动作，未获确认时返回"被拒"并让编排层走
  "需人工升级"出口。
- **库目录单写者锁**：两个进程同时写同一记忆库不保证安全，因此有库级写锁（第二个写者排队或被告知；僵尸锁可回收）。文档明示该限制，不假装支持并发写；**已验证**（CI live_management_smoke 端口探测与写锁排队测试通过）。
...
只扫工作树是**不完整**的：一条线索可以从三个出口进入公开面，八轮 V3 的闸只盖住了第一个。

| 出口 | 内容 | 清理代价 | 本轮处置 |
|---|---|---|---|
| **工作树**（tracked 文本） | 文档、源码、CI 配置 | 改一次提交即可 | 八轮 V3 起 `scan_public_leak.py` 全量扫，命中即红（要求**零**命中） |
| **git 对象**（提交信息、标签注解） | `git log %B`、`git tag -n` | 只能改写历史＋force push（不可逆例外，八轮 V2 已裁定**不改写**） | 九轮 W5：`--git-text` 纳入同一套规则；历史既有命中记为**基线条数**（`GIT_TEXT_BASELINE`，实测 2 条，出自 `2419926` 那条 message 的目录名与行号指针各 1），**只减不增**——新写一条即顶破基线 → 红 |
| **GitHub Release 正文** | `gh release view --json body` | 编辑 Release 即可（不改历史，代价小） | 同入 `--git-text`；本机有 gh 且可联网时实扫，CI 上 gh 不可用会**如实打印跳过**（不静默变绿） |

跑法（本地与 CI 同一命令）：

```bash
python scripts/scan_public_leak.py            # 只扫工作树（发布门槛：零命中）
python scripts/scan_public_leak.py --git-text # 加扫 git 对象与 Release（基线只减不增）
```
