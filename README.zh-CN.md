...
| 基准 | 官方指标 | 结果（95% CI） | 题量 |
|---|---|---|---|
| LongMemEval-oracle | LLM 判分准确率（官方 judge prompt） | **74.2%（70.2%–77.8%，Wilson）** | 全量 500 |
| LongMemEval-oracle（早先抽样批） | 同一官方判分 | 70.5%（63.8%–76.4%，Wilson） | 抽 200（留作口径历史，与全量批统计相容） |
...
同批检索口径（证据命中／答在文内）：LongMemEval 全量 500 题批 **99.6%**／40.0%（0 失败 0 跳过），
早先抽 200 题批 100.0%／48.5%（**两批不同源，别混着引**）；LoCoMo **45.5%**／17.4% 基线，
...
```bash
python -m pytest tests/ -q            # 536 条（含模型臂 21 条；C 盘紧张时加 --basetemp=D:/tmp/pt）
