...
哨兵（A5 / 六轮 G8）：--sentinel-threshold 45.0（神经档证据命中下限）——任一"全开"档结果
低于阈值则退出码 1（CI 用）；容差说明写进输出。默认只报告不红（--sentinel 才红）。

阈值口径（两档，别混）：
- 全量档（本机，神经档 bge-small）：阈值 **45.0** —— 基准 = LoCoMo 全量 1986 题证据命中
  45.7%（结果文档 §二；497 题抽样档实测 42.66%，留 ~1pp 容差）；
- CI 档（无网、零下载，词法档 builtin-hash + 仓内 fixture `tests/fixtures/locomo_conv26.json`，
  `--convs 1`）：本机标定全开证据命中 **31.66%**（199 题，n=199 标准误 ~±3.3pp）→
  CI 步骤用阈值 **25.0**（留 ~2σ 余量防脆，防 CI 机器差异误红）。
"""
